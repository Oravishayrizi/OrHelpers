...
#' bp + scale_color_palette2(index = c(4,5,7),labels=c("A","B","C"),name = "New Legend Title")
scale_color_palette2<-function(index=NULL,...){
    ...
    if (is.null(index)) {
        ...
        if (length(index)>length(mycol)) {
            warning("index length is too large, allowed maximum for the palette is 9 Returning the palette with that many colors")
            vals<-mycol
        }
